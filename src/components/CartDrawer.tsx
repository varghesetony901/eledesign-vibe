          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-brand-charcoal/5 bg-brand-cream">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold tracking-tight text-brand-forest">Your Cart</h2>
                <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-brand-forest/10 text-brand-forest">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  onClick={() => setIsCartOpen(false)}
                  className="p-1.5 rounded-full hover:bg-brand-charcoal/5 transition-colors focus:outline-none focus:ring-1 focus:ring-brand-forest"
              ...
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-brand-charcoal/60 max-w-[280px]">
                  ...
                  <Link
                    href="/shop"
                    onClick={() => setIsCartOpen(false)}
                    className="inline-flex items-center justify-center px-6 py-2.5 bg-brand-forest hover:bg-brand-forest-light text-white text-sm font-medium rounded-full transition-all duration-200 shadow-sm"
                  >
                    Browse Collections
                  </Link>
                </div>
                  ...
                  {cart.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="flex gap-4 py-4 first:pt-0 last:pb-0">
                        {/* Image */}
                        <div className="relative w-20 h-20 rounded-lg bg-brand-charcoal/5 overflow-hidden flex-shrink-0 border border-brand-charcoal/5">
                          {item.image ? (
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          <div>
                            <div className="flex justify-between items-start">
                              <h3 className="font-semibold text-brand-forest text-sm line-clamp-1">
                                {item.name}
                              </h3>
                              <button
                                onClick={() => removeFromCart(item.id)}
                                className="p-1 rounded-full text-brand-charcoal/40 hover:text-red-600 hover:bg-red-50 transition-all focus:outline-none"
                                aria-label="Remove item"
                              >
                                <Trash2 className="w-4 h-4" />
                              </button>
                            </div>
                            <p className="text-xs text-brand-charcoal/50 mt-0.5 capitalize">
                              {item.category || "Plant"}
                            </p>
                          </div>
              ...
              <div className="border-t border-brand-charcoal/10 p-6 bg-white space-y-4">
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-medium text-emerald-800">Calculated at Checkout</span>
                  </div>
                  <div className="flex justify-between items-end">
                    <span className="text-base font-medium text-brand-forest">Subtotal</span>
                    <div className="text-right">
                ...
                <div className="bg-brand-forest/5 border border-brand-forest/10 rounded-xl p-3 text-xs text-brand-forest flex flex-col gap-0.5">
                ...
                <div className="grid grid-cols-1 gap-2 pt-2">
                  <Link
                    href="/checkout"
                    onClick={() => setIsCartOpen(false)}
                    className="flex items-center justify-center gap-2 w-full py-3 bg-brand-forest hover:bg-brand-forest-light text-white font-medium rounded-full shadow-sm transition-all duration-200"
                  >
